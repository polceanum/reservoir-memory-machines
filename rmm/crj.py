"""
Implements cycle reservoirs with jumps (Rodan and Tino, 2012).
...
def setup_input_weight_matrix(n, m, v, start = 0):
    """ Sets up a fully connected input-to-reservoir connection matrix of
    size n x m with connection strength v and signs based on the
    digits of pi, starting at digit start.

    Args:
    n: The dimensionality of the input.
    m: The number of neurons.
    v: The absolute value for all entries.
    start (optional): The start digit of pi. This is relevant if this
       function is called multiple times.

    Returns:
    U: A m x n weight matrix with absolute value v in all entries and
       signs according ot the digits of pi.
    """
    U = np.ones((m, n)) * v
    signs = np.repeat(pi_digits, int((start + m*n)/1000) + 1)
    signs = np.sign(signs[start:(start + m*n)].reshape((m,n)) - 4.5)
    U *= signs
    return U

def setup_reservoir_matrix(m, w_c, w_j, l):
    """ Sets up a sparsely connected reservoir connection matrix
    of size m x m with a cycle of weight w_c and regular jumps of
    size l and weight w_j.

    Args:
    m:   The number of neurons.
    w_c: The weight on cycle connections. Must be in range [0, 1).
    w_j: The weight on jump connections. Must be in range [0, 1).
    l:   The length of jump connections. Must be an integer > 0.

    Returns:
    W:   A scipy.sparse.csr_matrix containing the recurrent connection
         weights. Each neuron is connected to the next one with weight
         w_c and to the lth-next one and lth previous one with weight
         w_j.
    """
    data = []
    rows = []
    ...
    for i in range(1,m):
        ...
        cols.append(i-1)
        data.append(w_c)
    # bi-directional jump connections
    for i in range(0, m-l, l):
        rows.append(i)
        cols.append((i+l) % m)
        data.append(w_j)
        rows.append((i+l) % m)
